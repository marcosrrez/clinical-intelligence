...
import { Activity, ShieldAlert, FileText, Send, Database, CheckCircle2, AlertTriangle, Info, TrendingUp, Target, Brain, RefreshCcw, Building, Mic, Square } from 'lucide-react';
      ...
      <div className="w-64 bg-slate-900 text-white flex flex-col p-6 shrink-0">
        ...
        <div className="mt-auto pt-6 border-t border-slate-800 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-xs font-bold shadow-inner">MR</div>
            <div className="flex flex-col">
              <span className="text-xs font-bold">Dr. Marcos</span>
              <span className="text-[10px] text-slate-500">Horizon Therapy</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="w-[600px] flex flex-col gap-6 overflow-y-auto pr-2 pb-8">
                ...
                <>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        {result.markers.primary_themes.map((theme, i) => (
                        <span key={i} className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded-md uppercase tracking-tight border border-slate-200">
                            #{theme}
                        </span>
                        ))}
                    </div>
                    ...
                    {result.audit.liability_flags.length > 0 && (
                        <div className="mb-4 space-y-2">
                        <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Liability Flags Detected:</p>
                        {result.audit.liability_flags.map((flag, idx) => (
                            <div key={idx} className="flex items-start gap-2 text-sm text-red-900 bg-red-100/50 p-2 rounded-md border border-red-200/50">
                    ...
                    <div className="space-y-2">
                        <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Clinical Suggestions:</p>
                        {result.audit.suggestions.map((sug, idx) => (
                        <div key={idx} className="flex items-start gap-2 text-sm text-slate-700 bg-white/50 p-2 rounded-md border border-slate-200/50">
                    ...
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center justify-between border-b border-slate-100 pb-2">
                        ...
                        <button onClick={saveSession} className="text-emerald-600 hover:text-emerald-700 normal-case font-bold hover:bg-emerald-50 px-3 py-1 rounded-md transition-colors flex items-center gap-1.5 border border-emerald-100">
                    ...
                    <div className="grid gap-8">
                        ...
                        <div key={key} className="group">
                            <h4 className="text-[10px] font-black text-slate-900 mb-2 uppercase tracking-widest opacity-30 group-hover:opacity-100 transition-opacity">{key}</h4>
                            <div className="text-[15px] text-slate-700 leading-relaxed bg-slate-50/50 p-4 rounded-xl border border-transparent group-hover:border-slate-100 group-hover:bg-slate-50 transition-all">
                            {value}
                            </div>
                        </div>
                        ))}
                ...
                <div className="flex-1 bg-slate-100/50 rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center p-12 text-center text-slate-400">
                    ...
                    <p className="max-w-xs text-sm leading-relaxed text-slate-500 font-medium">Submit session raw notes to initiate the reflective multi-agent audit loop and longitudinal pattern extraction.</p>
                </div>
                )}
            </div>
            </main>
